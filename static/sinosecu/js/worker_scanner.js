	...
	} else if (msgNotify.Command == 'Save') 
	{
		if (msgNotify.Operand == 'CardContentText') {
			...
			if(isPassport){
				...
				var docFiled = document.getElementById(`id_documents-${formIdxNew-1}-document_file`);
				let docTypeNID = document.getElementById(`id_documents-${formIdxNew-1}-document_type`);

				if(docType.value == "passport" && docTypeNID.value != "passport" && docTypeNID.value != "id_card" ){
				
					return false;
				}
				//document generate
				if(docType.value == "passport" && docTypeNID.value == "id_card"){
...
function tryDisplayImage(images, imageName, domId) {
	if (images.hasOwnProperty(imageName)) {
		...
		}else if(isIDCard && isKhmerIDCard){
			document.getElementById("defaultImg").style.display = "none";
			document.getElementById("imageWhite").style.display = "block";
			document.getElementById("previewImg").style.display = "block";
			document.getElementById("imageWhite").src = images["White"];
...
function getContent(text){
	...
	if(isPassport){
		...
		if(text["Date of issue (only PRC)"] == null){
			...
			docIssueDate.style.color = "#131414";
			docIssueDate.style.fontWeight = "normal";
			docIssueDate.style.border = "1px solid gray";
			docIssueDate.style.textAlign = "left";
	...
	if(isIDCard){
		...
		//check worker is khmer
		if(text["Country code(Issuing state)"].slice(0, -1) == "KH"){
			//set isKhmerIDCard
			isKhmerIDCard = true
			...
			//check id card duplicated
			checkPassportDuplicate(text["ID Number"])
			//document generate
			...
			docIssueDate.value  = getIssueDateFromExpired(expiryDate);
			...
			// id card not khmer
			let formIdxNID = parseInt(document.querySelector('#id_documents-TOTAL_FORMS').value);
		
			//check passport duplicated
			checkPassportDuplicate(text["ID Number"])

			//document generate
			let docTypeNID = document.getElementById(`id_documents-${formIdxNID-1}-document_type`);
			var docNoNID = document.getElementById(`id_documents-${formIdxNID-1}-document_number`);
			var docIssueDateNID = document.getElementById(`id_documents-${formIdxNID-1}-issue_date`);
			var docExpiryDateNID = document.getElementById(`id_documents-${formIdxNID-1}-expiry_date`);
			var docIssueAuthorityNID = document.getElementById(`id_documents-${formIdxNID-1}-issuing_authority`);
			var expiryDateLabelErrorNID = document.getElementById(`expiry-date-error-${formIdxNID-1}`);
			var issueDateErrorNID = document.getElementById(`issues-date-error-${formIdxNID-1}`);
			
			// check is document type is id card
			if(docTypeNID.value == "" && docType.value == "passport"){
				docTypeNID.value = "id_card";
			}
			if(docType.value == "passport" && docTypeNID.value != "passport" && docTypeNID.value != "id_card" ){
				toastAlert.error("Document scan is id card but document type is not id card");
				return false;
			}
			//document generate
			if(docType.value == "passport" && docTypeNID.value == "id_card"){
				...
				docTypeNID.value = "id_card";
				docNoNID.value = text["ID Number"];

				docExpiryDateNID.value = text["Date of Expiry"];
				docIssueAuthorityNID.value = text["Country code(Issuing state)"];

				//check expiry date
			
				var expiryDate = new Date(text["Date of Expiry"]);

				var issuesDate = getIssueDateFromExpired(expiryDate);
		
				docIssueDateNID.value  = issuesDate

				if(expiryDate < today){

					docExpiryDateNID.value =null;
					docExpiryDateNID.style.color = "red";
					docExpiryDateNID.style.fontWeight = "bold";
					docExpiryDateNID.style.border = "1px solid red";
					docExpiryDateNID.style.textAlign = "left";
					expiryDateLabelErrorNID.innerHTML = " ( Expiry date is expired ) ";
					if (expiredDateErrorNID) {
						expiredDateErrorNID.classList.remove("hidden");
					}
					// btnCreateWorker.setAttribute("disabled", "disabled");
					// btnCreateWorker.classList.add("disabled");
					return;
					...
					docExpiryDateNID.value = text["Date of Expiry"];
					expiryDateLabelErrorNID.innerHTML = "";
					if (expiredDateErrorNID) {
						expiredDateErrorNID.classList.add("hidden");
					}
					// btnCreateWorker.setAttribute("disabled", "disabled");
					// btnCreateWorker.classList.remove("disabled");
				}
...
const toastAlert = {
	show: function(message, type = 'info', duration = 5000) {
		...
		const toastHtml = `
			<div id="${toastId}"  class="toast flex items-start w-full max-w-xs py-3 px-4 text-gray-500 bg-white rounded-2xl shadow border border-gray-100">
				<div class="inline-flex items-center justify-center flex-shrink-0 w-6 h-6 text-white bg-${type === 'error' ? 'red-500' : type === 'warning'? 'yellow-500': type==='info'?'blue-500':'green-500'} ring-4 ring-${type === 'error' ? 'red-100' : type === 'warning'? 'yellow-100': type==='info'?'blue-100':'green-100'} rounded-full">
					<i class="bi ${iconMap[type]} icon-md text-white"></i>
				</div>
				<div class="ml-3 text-sm font-normal">${message}</div>
				<button type="button" class="btn-close ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg p-1 hover:bg-gray-100 inline-flex h-4 w-4" data-bs-dismiss="toast" aria-label="Close"></button>
			</div>
			`;
...
function getIssueDateFromExpired(expiredDateStr) {
  // Parse the expired date (expected format: 'YYYY-MM-DD')
  const expiredDate = new Date(expiredDateStr);

  // Subtract 10 years
  const issueDate = new Date(expiredDate);
